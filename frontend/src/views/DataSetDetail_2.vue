...
<template>  
    ...
    <div v-if="dataset">
      ...
      <div class="detail-item">
        ...
        <span v-if="tags.length">
          <span v-for="tag in tags" :key="tag" class="tag" @click="goToSearchWithTag(tag.id)">{{ tag.name }}</span>
        </span>
        <span v-else>none</span>
      </div>
      <div class="detail-item"><strong>upload time:</strong> {{ formatDate(dataset.uploadTime) }}</div>
  ...
  <div class="comment-list" v-if="comments.length">
    ...
    >
      ...
      <p class="comment-meta" style="font-size: 13px; color: #666; margin: 5px 0;">
        from: <strong>{{ comment.username }}</strong> date: {{ formatDate(comment.createdAt) }}
        <button 
          v-if="comment.userId === currentUserId"
          @click="deleteComment(comment.id)"
          class="delete-button"
          style="margin-left: 15px; color: white; background-color: red; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer;"
        >
          delete
        </button>
      </p>
...
export default {
  ...
  methods: {
    goToSearchWithTag(tagId) {
      this.$router.push({ 
        path: '/SearchDataSet', 
        query: { tag: tagId ,from:'detail_tag'} 
      });
    },
    ...
    getCurrentUserId() {
      ...
      if (token) {
        ...
        this.currentUserId = parseInt(decoded.userId);
      }
    ...
    async deleteComment(commentId) {
      if (confirm('Are you sure to delete?')) {
        try {
          const token = localStorage.getItem('jwtToken');
          await axios.delete(`/dataset/comments/${commentId}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          this.comments = this.comments.filter(c => c.id !== commentId);
          this.$message?.success?.('comment deleted') || alert('comment deleted');
        } catch (err) {
          this.$message?.error?.('fail') || alert('fail');
        }
      }
    },
    ...
    goBack() {
      this.$router.push({ name: 'Mainpage', query: { activeView: 'SearchDataSet' } });
    }
  }
};
</script>
